...
describe("judge provider-free presentation lineage", () => {
  ...
  it("verifies the exact e2-style sealed-reader recovery and active checkout", async () => {
    ...
    for (const [judgePack, expectedStatus] of [
      [undefined, 0],
      ["AA", 1]
    ] as const) {
      ...
      if (expectedStatus === 0) {
        ...
        expect(production.stdout).toContain('"gitProofTransport":"full-local-history"');
      } else {
        expect(production.status).not.toBe(0);
        expect(production.stderr).toContain("judge_demo_presentation_dedicated_git_pack_forbidden");
      }
    }
