...
describe("Gate 6 terminal presentation proof", () => {
  it("accepts only sorted allowlisted presentation changes bound to unchanged critical files", async () => {
    ...
    for (const originAliasPath of [
      ...JUDGE_DEMO_ORIGIN_ALIAS_PROTOCOL_PATHS,
      ...JUDGE_DEMO_ORIGIN_ALIAS_IMPLEMENTATION_PATHS
    ]) {
      expect(gate6PresentationPathAllowed(originAliasPath), originAliasPath).toBe(true);
    }
    expect(await canonicalSha256(JUDGE_DEMO_ORIGIN_ALIAS_PROTOCOL_PATHS)).toBe(
      JUDGE_DEMO_ORIGIN_ALIAS_PROTOCOL_PATHS_HASH
    );
    expect(await canonicalSha256(JUDGE_DEMO_ORIGIN_ALIAS_IMPLEMENTATION_PATHS)).toBe(
      JUDGE_DEMO_ORIGIN_ALIAS_IMPLEMENTATION_PATHS_HASH
    );
