...
The first four calibration calls are retained as a separate authentic failed attempt. Sergio approved one policy migration that reallocated four unused judge/reference grants without changing the guard identity, global limit, spend ceiling, or any existing call/cost record. A single atomic Lua transition requires the exact old guard and four `KNOWN` call/provider records, preserves every counter and sequence, changes only the versioned policy/script identities plus calibration/judge limits, and writes a permanent replay-safe receipt. It never initializes, reaps, resets, relabels, or deletes prior state.

The dedicated claimed durable store, separate random signing and activation secrets, Production-only provider/store credentials, real concurrent Lua verification, and immutable guard receipt are established. The implemented Gate 2 route requires one exact production project/build, operator-pinned policy/Lua/runner hashes, the environment-pinned durable migration receipt, a valid activation HMAC, an open internally consistent guard, a short-lived HttpOnly Secure SameSite=Strict session, and a constant-time matching CSRF header. Without every binding, all model routes return `503 probe_disabled` and make no inference.
