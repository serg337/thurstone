...
**60-second path:** in Chrome 149+, enable `chrome://flags/#enable-webmcp-testing` and relaunch, then open the live Lab signed out → confirm the exact four-tool `consumer-ready` catalog → run the single fixed judge decision and native `cart_get` → inspect or download the sealed receipt → open Results for the `23/24 → 23/24` paired evidence. The other supported Site Tools path is the latest ChatGPT desktop built-in browser with GPT-5.6 Sol or Terra.

**Supported-path status:** native Chrome WebMCP and four fresh official ChatGPT desktop built-in-browser Site Tools observations are verified. The bounded server-side OpenAI judge lane requires no judge key. Its one-call receipt is intentionally deployment-bound: the live Lab/status download and release record—not mutable source prose—report whether it is available, recoverable, or sealed.
...
| Judge-accessible model-backed lane                             | One fixed signed-out read; live status reports the durable one-call/sealed-replay state    |
...
| `TOOLPROOF_JUDGE_PRESENTATION_BINDING_B64`, `TOOLPROOF_JUDGE_PRESENTATION_BINDING_HASH`                                  | Provider-free predecessor/successor archive binding; absent on evidence build         |
| `TOOLPROOF_JUDGE_GIT_PACK_B64`                                                                                           | Optional build-only successor Git objects; full verified history is equivalent        |
