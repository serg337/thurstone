import { canonicalJson, canonicalSha256 } from "@/lib/evidence/digest";
import {
  createFallbackModelInput,
  parseExpectationFreeFallbackEnvelope,
  verifyFallbackTransportBinding,
  type FallbackCalibrationEnvelope
} from "@/lib/fallback/calibration-envelope";
import {
  FALLBACK_GENERIC_RUNNER_PROMPT,
  FALLBACK_RUNNER_PROMPT_VERSION,
  FALLBACK_RUNNER_SETTINGS_MANIFEST,
  FALLBACK_RUNNER_SETTINGS_VERSION,
  fallbackBrowserRuntimeContractHash,
  fallbackRunnerPromptHash,
  fallbackRunnerSettingsHash
} from "@/lib/fallback/runner-contract";
import {
  createProbeFunctionToolDefinitions,
  parseProbeDecision,
  parseProbeDecisionOutput,
  probeFunctionToolDefinitionsHash,
  type ProbeDecision
} from "@/lib/probe/decision";
import { PROBE_MAX_OUTPUT_TOKENS, PROBE_MODEL } from "@/lib/probe/policy";
import { assertNoProbeExpectationLeakage } from "@/lib/probe/calibration-envelope";
import { z } from "zod";

export const FALLBACK_NO_CALL_SCHEMA_VERSION = "toolproof-fallback-no-call-schema@1.0.0";
export const FALLBACK_TOOL_DECISION_REQUEST_VERSION =
  "toolproof-fallback-tool-decision-request@1.0.0";
export const FALLBACK_TOOL_DECISION_RECEIPT_VERSION =
  "toolproof-fallback-tool-decision-receipt@1.0.0";

const responseUsageSchema = z
  .object({
    input_tokens: z.number().int().nonnegative(),
    output_tokens: z.number().int().nonnegative(),
    total_tokens: z.number().int().nonnegative()
  })
  .passthrough();

const responseEnvelopeSchema = z
  .object({
    id: z.string().min(1).max(256),
    object: z.literal("response"),
    model: z.string().min(1).max(256),
    status: z.enum(["completed", "failed", "in_progress", "cancelled", "queued", "incomplete"]),
    output: z.array(z.unknown()).max(32),
    usage: responseUsageSchema.nullable()
  })
  .passthrough();

export class FallbackToolDecisionError extends Error {
  constructor(readonly code: string) {
    super(code);
    this.name = "FallbackToolDecisionError";
  }
}

function noCallBranch(kind: "clarify" | "abstain", field: "text" | "reason") {
  return {
    type: "object",
    properties: {
      kind: { type: "string", enum: [kind] },
      [field]: { type: "string" }
    },
    required: ["kind", field],
    additionalProperties: false
  };
}

export function createFallbackNoCallJsonSchema() {
  return Object.freeze({
    version: FALLBACK_NO_CALL_SCHEMA_VERSION,
    name: "toolproof_fallback_no_call_v1",
    strict: true as const,
    schema: Object.freeze({
      type: "object",
      properties: {
        decision: {
          anyOf: [noCallBranch("clarify", "text"), noCallBranch("abstain", "reason")]
        }
      },
      required: ["decision"],
      additionalProperties: false
    })
  });
}

export function fallbackNoCallJsonSchemaHash(): Promise<string> {
  return canonicalSha256(createFallbackNoCallJsonSchema());
}

export interface PreparedFallbackToolDecisionRequest {
  readonly version: typeof FALLBACK_TOOL_DECISION_REQUEST_VERSION;
  readonly envelope: FallbackCalibrationEnvelope;
  readonly body: Readonly<Record<string, unknown>>;
  readonly requestBodyBytes: string;
  readonly toolDefinitionsHash: string;
  readonly noCallSchemaHash: string;
}

export async function createFallbackToolDecisionRequest(input: {
  readonly envelope: FallbackCalibrationEnvelope;
  readonly safetyIdentifier: string;
}): Promise<PreparedFallbackToolDecisionRequest> {
  if (!/^[a-f0-9]{64}$/u.test(input.safetyIdentifier)) {
    throw new FallbackToolDecisionError("invalid_safety_identifier");
  }
  const envelope = parseExpectationFreeFallbackEnvelope(input.envelope);
  const transport = await verifyFallbackTransportBinding(envelope);
  const modelInput = createFallbackModelInput(envelope);
  const tools = createProbeFunctionToolDefinitions(envelope.liveManifest, transport);
  const noCall = createFallbackNoCallJsonSchema();
  const [promptHash, settingsHash, browserRuntimeHash, toolDefinitionsHash, noCallSchemaHash] =
    await Promise.all([
      fallbackRunnerPromptHash(),
      fallbackRunnerSettingsHash(),
      fallbackBrowserRuntimeContractHash(),
      probeFunctionToolDefinitionsHash(envelope.liveManifest, transport),
      fallbackNoCallJsonSchemaHash()
    ]);
  if (
    envelope.runner.promptVersion !== FALLBACK_RUNNER_PROMPT_VERSION ||
    envelope.runner.promptHash !== promptHash ||
    envelope.runner.settingsVersion !== FALLBACK_RUNNER_SETTINGS_VERSION ||
    envelope.runner.settingsHash !== settingsHash ||
    envelope.runner.browserRuntimeHash !== browserRuntimeHash ||
    envelope.runner.toolDefinitionsHash !== toolDefinitionsHash ||
    envelope.runner.noCallSchemaHash !== noCallSchemaHash
  ) {
    throw new FallbackToolDecisionError("fallback_runner_binding_mismatch");
  }
  const body = Object.freeze({
    model: PROBE_MODEL,
    instructions: FALLBACK_GENERIC_RUNNER_PROMPT,
    input: canonicalJson(modelInput),
    reasoning: { effort: FALLBACK_RUNNER_SETTINGS_MANIFEST.reasoningEffort },
    tools,
    tool_choice: "auto",
    parallel_tool_calls: false,
    text: {
      format: {
        type: "json_schema",
        name: noCall.name,
        strict: noCall.strict,
        schema: noCall.schema
      },
      verbosity: "low"
    },
    max_output_tokens: PROBE_MAX_OUTPUT_TOKENS,
    store: false,
    truncation: "disabled",
    service_tier: "default",
    safety_identifier: input.safetyIdentifier
  });
  assertNoProbeExpectationLeakage(body);
  return Object.freeze({
    version: FALLBACK_TOOL_DECISION_REQUEST_VERSION,
    envelope,
    body,
    requestBodyBytes: canonicalJson(body),
    toolDefinitionsHash,
    noCallSchemaHash
  });
}

function objectValue(value: unknown): Record<string, unknown> | null {
  return value && typeof value === "object" && !Array.isArray(value)
    ? (value as Record<string, unknown>)
    : null;
}

function extractResponseDecision(output: readonly unknown[]): {
  readonly functionCalls: readonly Record<string, unknown>[];
  readonly outputTexts: readonly string[];
  readonly refusals: readonly string[];
} {
  const functionCalls: Record<string, unknown>[] = [];
  const outputTexts: string[] = [];
  const refusals: string[] = [];
  for (const item of output) {
    const object = objectValue(item);
    if (!object) continue;
    if (object.type === "function_call") {
      functionCalls.push(object);
      continue;
    }
    if (object.type !== "message" || !Array.isArray(object.content)) continue;
    for (const entry of object.content) {
      const content = objectValue(entry);
      if (content?.type === "output_text" && typeof content.text === "string") {
        outputTexts.push(content.text);
      }
      if (content?.type === "refusal" && typeof content.refusal === "string") {
        refusals.push(content.refusal);
      }
    }
  }
  return { functionCalls, outputTexts, refusals };
}

export interface FallbackToolDecisionReceipt {
  readonly version: typeof FALLBACK_TOOL_DECISION_RECEIPT_VERSION;
  readonly responseId: string;
  readonly responseStatus: string;
  readonly model: typeof PROBE_MODEL;
  readonly decision: ProbeDecision | null;
  readonly decisionError: string | null;
  readonly refusal: string | null;
  readonly toolCallId: string | null;
  readonly rawArgumentsBytes: string | null;
  readonly outputText: string | null;
  readonly providerCallCount: 1;
  readonly toolCallCount: number;
  readonly store: false;
  readonly previousResponseId: null;
  readonly conversationId: null;
  readonly usage: z.infer<typeof responseUsageSchema> | null;
}

export function parseFallbackToolDecisionResponse(
  value: unknown,
  envelopeValue: FallbackCalibrationEnvelope
): FallbackToolDecisionReceipt {
  const envelope = parseExpectationFreeFallbackEnvelope(envelopeValue);
  const parsed = responseEnvelopeSchema.safeParse(value);
  if (!parsed.success || parsed.data.model !== PROBE_MODEL) {
    throw new FallbackToolDecisionError("invalid_provider_envelope");
  }
  const extracted = extractResponseDecision(parsed.data.output);
  let decision: ProbeDecision | null = null;
  let decisionError: string | null = null;
  let refusal: string | null = null;
  let toolCallId: string | null = null;
  let rawArgumentsBytes: string | null = null;
  let outputText: string | null = null;

  if (parsed.data.status !== "completed") {
    decisionError = `provider_${parsed.data.status}`;
  } else if (extracted.refusals.length > 0) {
    refusal = extracted.refusals.join("\n").slice(0, 4_000);
    decisionError = "provider_refusal";
  } else if (extracted.functionCalls.length === 1 && extracted.outputTexts.length === 0) {
    const call = extracted.functionCalls[0]!;
    if (
      typeof call.name !== "string" ||
      typeof call.arguments !== "string" ||
      typeof call.call_id !== "string"
    ) {
      decisionError = "invalid_function_call";
    } else {
      toolCallId = call.call_id;
      rawArgumentsBytes = call.arguments;
      try {
        decision = parseProbeDecision(
          { kind: "call", tool: call.name, arguments: JSON.parse(call.arguments) as unknown },
          envelope.liveManifest,
          envelope.runner.transport
        );
      } catch {
        decisionError = "invalid_function_call";
      }
    }
  } else if (extracted.functionCalls.length === 0 && extracted.outputTexts.length === 1) {
    outputText = extracted.outputTexts[0]!;
    try {
      const parsedDecision = parseProbeDecisionOutput(
        JSON.parse(outputText) as unknown,
        envelope.liveManifest,
        envelope.runner.transport
      );
      if (parsedDecision.kind === "call") throw new Error("text_call_forbidden");
      decision = parsedDecision;
    } catch {
      decisionError = "invalid_no_call_output";
    }
  } else {
    decisionError = "invalid_decision_output_count";
  }

  return Object.freeze({
    version: FALLBACK_TOOL_DECISION_RECEIPT_VERSION,
    responseId: parsed.data.id,
    responseStatus: parsed.data.status,
    model: PROBE_MODEL,
    decision,
    decisionError,
    refusal,
    toolCallId,
    rawArgumentsBytes,
    outputText,
    providerCallCount: 1 as const,
    toolCallCount: extracted.functionCalls.length,
    store: false as const,
    previousResponseId: null,
    conversationId: null,
    usage: parsed.data.usage
  });
}
