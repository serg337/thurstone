...
function scoredRows(snapshot: ScoredRunSnapshot): readonly SemanticDevelopmentResultRow[] {
  ...
  for (const attempt of snapshot.attempts) {
    ...
    const definition = GATE3_SEMANTIC_SUITE.scoredCases.find(
...
export async function readSemanticResults(
  ...
  if (!runId && !evidenceDigest) {
    return Object.freeze({ status: "no-scored-run", disclosure: "No run yet" });
  }
  if (
  ...
  const baselineRows = scoredRows(snapshot);
  ...
  if (revisedRunId || revisedEvidenceDigest) {
    ...
    const revisedRows = scoredRows(revisedSnapshot);
