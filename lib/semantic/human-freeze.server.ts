...
export interface Gate3FrozenProtocol {
  readonly version: typeof GATE3_FROZEN_PROTOCOL_VERSION;
...
export async function finalizeGate3HumanFreeze(input: {
  ...
  const [humanReviewReceiptHash, authoringTerminationHash] = await Promise.all([
    canonicalSha256(humanReviewReceipt),
    canonicalSha256(authoringTermination)
  ]);
  const frozenManifest = {
    ...review.freezeManifest,
    status: "frozen" as const
  };
  const payload: Omit<Gate3FrozenProtocol, "frozenProtocolHash"> = {
